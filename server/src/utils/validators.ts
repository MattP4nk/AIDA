...
export function validateCommand(command: string): { isValid: boolean; error?: string } {
  ...
  if (command.length > 1000) {
    return { isValid: false, error: 'Command is too long (max 1000 characters)' };
  }
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(command)) {
      return { isValid: false, error: 'Command contains potentially dangerous patterns' };
    }
  }
 ...
 * @returns Object with isValid flag and optional error message
 */
export function validateArgs(args: string[]): { isValid: boolean; error?: string } {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.length > 500) {
      return { isValid: false, error: `Argument ${i} is too long (max 500 characters)` };
    }
 ...
 * @returns True if valid
 */
export function validateNumberInRange(value: any, min: number, max: number): boolean {
